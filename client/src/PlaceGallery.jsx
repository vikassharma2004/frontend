      ...
      <div className="absolute inset-0 bg-black text-white min-h-screen ">
        <div className="bg-black p-8 grid gap-4">
          <div>
            ...
            <button onClick={() => setShowAllPhotos(false)} className="fixed right-4 top-8 flex gap-1 py-1 px-1 rounded-2xl shadow shadow-black  text-black  hover:bg-white">
          ...
          {place?.photos?.length > 0 && place.photos.map(photo => (
            <div>
              <Image src={photo} alt="" className="rounded-2xl"/>
            </div>
          ))}
        </div>
      </div>
